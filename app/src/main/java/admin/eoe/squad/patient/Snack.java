...
public class Snack extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        lv = (ListView) findViewById(R.id.snack_lv);
        save = (Button) findViewById(R.id.snack_button_save);
        add = (Button) findViewById(R.id.snack_button);
        database = FirebaseDatabase.getInstance().getReference(FirebaseAuth.getInstance().getCurrentUser().getUid());

        database.child("snacksub").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        add.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(getApplicationContext(),SnackSub.class));
            }
        });
