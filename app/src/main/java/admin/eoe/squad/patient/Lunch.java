...
public class Lunch extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        lv = (ListView) findViewById(R.id.lunch_lv);
        save = (Button) findViewById(R.id.lunch_button_save);
        add = (Button) findViewById(R.id.lunch_button);
        database = FirebaseDatabase.getInstance().getReference(FirebaseAuth.getInstance().getCurrentUser().getUid());

        database.child("lunchsub").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        add.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(getApplicationContext(),LunchSub.class));
            }
        });
